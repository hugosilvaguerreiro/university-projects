...
import aasma_proj.tmp.Block.Shape;

/**
 * aasma_proj.tmp.Agent behavior
...
public class Agent extends Entity {
	...
	/* Accesses the states of an agent given its position, direction and cargo */
	public int getState(Point p, int direction, Box box) {
		int v = p.x*Board.nY+p.y; //coordinates
		v += (direction/90)*Board.nX*Board.nY; //direction
		if(box!=null) {
			for(int i=0; i<Board.colors.length; i++)
				if(box.color.equals(Board.colors[i])) {
					v += (i+1)*directions*Board.nX*Board.nY; //box
					break;
				}
		}
		return v;
	...
	private Action eGreedySelection() {
		...
		if(random.nextDouble()>epsilon) 
			return actions.get(validActions.get(random.nextInt(validActions.size())));
		else return getMaxActionQ(getState(point,direction,cargo),validActions);
	}
	...
	/* Get action with higher likelihood for a given states from q */
	...
	/* Gets the maximum Q-value action for a states (x y) */
	...
	/* Gets the Q-value for a specific states-action pair (x y action) */
	private double getQ(int state, Action action) {
		return q[state][actions.indexOf(action)];
	}

	/* Sets the Q-value for a specific states-action pair (x y action) */
	...
	public void grabBox() {
	  ...
	  cargo.grabBox(point);
	  //aasma_proj.tmp.Board.sendMessage(Action.grab, ahead);
	}
	...
	public void dropBox() {
		cargo.dropBox(ahead);
	    cargo = null;
		//aasma_proj.tmp.Board.sendMessage(Action.drop, ahead);
	}
