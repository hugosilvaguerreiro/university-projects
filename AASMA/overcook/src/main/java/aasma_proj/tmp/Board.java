...
public class Board {
	...
	public static void initialize() {
		...
		for(int i=0; i<nX; i++) 
			for(int j=0; j<nY; j++) 
				board[i][j] = new Block(Block.Shape.free, Color.lightGray);
		...
		for(int i=rampX, k=0; i<2*rampX; i++) {
			for(int j=0; j<rampY; j++) {
				board[i-1][j] = new Block(Block.Shape.ramp, Color.gray);
		...
		for(int k=0; k<pshelves.length; k++) 
			for(int i=0; i<2; i++) 
				board[pshelves[k].x+i][pshelves[k].y] = new Block(Block.Shape.shelf, colors[k]);
	...
	public static void sendMessage(Point point, Block.Shape shape, Color color, boolean free) {
