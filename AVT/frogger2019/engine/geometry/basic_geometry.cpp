...
void createQuad(MyMesh *obj_mesh, float size_x, float size_y) {
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned int) * obj_mesh->numIndexes, quad_faceIndex , GL_STATIC_DRAW);
...
void createCar(MyMesh *mesh) {
    //obj_mesh->numIndexes = faceCount *3;
}
...
void createSphere(MyMesh *obj_mesh, float radius, int divisions) {
	...
	computeVAO(obj_mesh, divisions+1, p+2, p, divisions*2, 0.0f);
}
...
void createTorus(MyMesh *obj_mesh, float innerRadius, float outerRadius, int rings, int sides) {
	...
	computeVAO(obj_mesh, sides+1, p+2, p, rings, 0.0f);
}
...
void createCylinder(MyMesh *obj_mesh,float height, float radius, int sides) {
	...
	computeVAO(obj_mesh, 4, p+2, p, sides, 0.0f);
}
...
void createCone(MyMesh *obj_mesh, float height, float baseRadius, int sides) {
	...
	computeVAO(obj_mesh, (p.size()-4)/2, &(p[2]), &(p[0]), sides, 0.0f);
}
...
void createPawn(MyMesh *obj_mesh) {
	...
	computeVAO(obj_mesh, numP, p, points, sides, smoothCos);
}

void computeVAO(MyMesh *obj_mesh, int numP, float *p, float *points, int sides, float smoothCos) {
