...
function Entity(data, display, options, image, menu) {
...
World.prototype = {
  addEntity: function(data, options, image) {
      ...
      for (var aCount = 0; aCount < entitiesLen; aCount++) {
        ...
        for (var bCount = aCount + 1; bCount < entitiesLen; bCount++) {
          ...
          if (collided) {
            ...
            if(a.image && a.isSolid) {
              if(aData instanceof SAT.Circle) {
                a.image.attr({"x":a.data.pos.x - a.data.r, "y":a.data.pos.y - a.data.r});
              }
              else {
                a.image.attr({"x":a.data.pos.x, "y":a.data.pos.y});
              }
              if(a.menu)
                  move(a.menu, {x : a.data.pos.x, y : a.data.pos.y});
            }
            if(b.image &&  b.isSolid) {
              if(bData instanceof SAT.Circle) {
                b.image.attr({"x":b.data.pos.x - b.data.r, "y":b.data.pos.y - b.data.r});
              }
              else {
                b.image.attr({"x":b.data.pos.x, "y":b.data.pos.y});
              }
              if(b.menu)
                move(b.menu, {x : b.data.pos.x, y : b.data.pos.y});
            }
          }
        }
      }
...
function move(menu, pos) {
        ...
        $("#"+ menu.button).css({top: pos.y});
        $("#"+ menu.button).css({left:pos.x + menu.element.attr("width") - 31});        
}